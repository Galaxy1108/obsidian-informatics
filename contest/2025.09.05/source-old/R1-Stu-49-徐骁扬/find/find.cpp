...
long long N,sd,K,len,i,j,k,l,m,n;
long long f[12][10][40][30][20][20];
long long g[12][10][40][30][20][20];
long long s[120010],ans,mem;
...
int main()
{
	...
	while(N)
	{
		...
		for(m=da[i][3];m<=len;m++)
			g[len][i][j][k][l][m]+=g[len-1][n][j-da[i][0]][k-da[i][1]][l-da[i][2]][m-da[i][3]];
		i=N%10;	
		for(j=da[i][0];j<=3*len;j++)
		for(k=da[i][1];k<=2*len;k++)
		for(l=da[i][2];l<=len;l++)
		...
		{
			for(n=1;n<a[len-1];n++)
				f[len][i][j][k][l][m]+=g[len-1][n][j-da[i][0]][k-da[i][1]][l-da[i][2]][m-da[i][3]];
			f[len][i][j][k][l][m]+=f[len-1][a[len-1]][j-da[i][0]][k-da[i][1]][l-da[i][2]][m-da[i][3]];
		}
		a[len]=N%10;
		N/=10;
	}
	...
	for(n=1;n<=9;n++)
	{
		...
		for(l=0;l<=1*i;l++)
		for(m=0;m<=1*i;m++)
			s[j*23*12*12+k*12*12+l*12+m]+=g[i][n][j][k][l][m];
	}
	for(n=1;n<a[len];n++)
	{
		for(j=0;j<=3*i;j++)
	...
	for(l=0;l<=1*i;l++)
	for(m=0;m<=1*i;m++)
		s[j*23*12*12+k*12*12+l*12+m]+=f[len][a[len]][j][k][l][m];
	sort(s,s+113000,cmp);
//	for(i=0;s[i];i++)
//		printf("%d ",s[i]);
	sta.id1=0;sta.id2=0;
