...
stack<int> a , b ;
int n = 0 , w[100005] = {} , h[100005] = {} , ans = 0 ;
int calc(int x)
{
...
signed main()
{
	...
	for(int i = 1 ; i <= n ; ++i)
		qcin>>w[i] ;
	for(int i = 1 , r = 0 ; i <= n + 1 ; ++i , r = 0)
	{
		while(h[i] <= a.top())
		{
			int hnow = a.top() , wnow = b.top() ;
			a.pop() , b.pop() ;
			(r += wnow) %= mod ;
			(ans += ((calc(hnow) - calc(max(h[i] , a.top())) + mod) % mod * calc(r))) %= mod ;
		}
		a.push(h[i]) , b.push((w[i] + r) % mod) ;
	}
	qcout<<ans<<'\n' ;
	return 0;
}
