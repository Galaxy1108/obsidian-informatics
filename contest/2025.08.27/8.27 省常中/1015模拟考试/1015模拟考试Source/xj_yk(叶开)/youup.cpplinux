...
// 于是对于不会的包，直接输出一手组合数。
// 大概会 O(n^3) 的包？
// 维护每个节点子树中已经合法时，最深节点深度以及对应点数已知时，其方案数。
// 合并时维护表示当前最大深度，同时保证两部分深度和不超过 x。
// 对点数维进行卷积，然后再叠加上原有贡献。
// 至于长剖优化……感觉很不方便啊。
// 怎么办办。
// 不管了，打暴力吧。
...
int main()
{
    ...
    uint t,n,x;scanf("%u%u%u",&t,&n,&x);
    if(!x){
        for(uint i=1;i<=n;i++)printf("%u%c",i==1?n:0," \n"[i==n]);
        return 0;
    }
    if(x==1){
        for(uint i=1;i<=n;i++)
            printf("%u%c",(i==1?n:(i==2?n-1:0))," \n"[i==n]);
        return 0;
    }
    if(t==4){
        for(uint i=1;i<=n;i++)
            (P[n]*Q[i]*Q[n-i]).print(),putchar(" \n"[i==n]);
        return 0;
    }
    for(uint i=1;i<=n;i++)
        (P[n]*Q[i]*Q[n-i]).print(),putchar(" \n"[i==n]);
