...
modint P[1000005],Q[1000005],R[1000005];
int main()
{
#ifdef MYEE
...
#endif
    uint t,k;scanf("%u%u",&k,&t);k++;
    P[k-1]=k,P[k]=modint(1)-k;
    for(uint i=k;i<=1000000;i++)P[i]+=P[i-1];
    for(uint i=k;i<=1000000;i++)P[i]+=P[i-1]+P[i-k];
    for(uint i=1000000;i;i--)P[i]=P[i-1],Q[i]=i;
    P[0]=0;
    for(uint i=2;i<=1000000;i++)
    {
        static bol Gone[1000005];
        if(!Gone[i])for(uint j=i;j<=1000000;j+=i)
            Q[j]=Q[j]()/i*(i-1),Gone[j]=1;
    }
