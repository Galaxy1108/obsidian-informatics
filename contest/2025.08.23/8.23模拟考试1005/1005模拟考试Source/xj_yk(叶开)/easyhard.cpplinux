...
modint A[3005][3005],Last[3005][3005];
int main(){
#ifdef MYEE
...
#endif
    uint n,m;ullt p;scanf("%u%u%llu",&n,&m,&p);AnyMod::ChgMod(p);
    if(m<=n){
        (modint(4)^m).println();
        return 0;
    }
    ...
    modint ans;
    for(uint i=0;i<=n;i++)for(uint j=0;i+j<=n;j++)ans+=A[i][j];
    ans.println();
    return 0;
}
