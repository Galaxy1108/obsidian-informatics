...
using namespace IO ;
#define inf 1e9
int n = 0 , w[10005] = {} , u[10005] = {} , t[10005] = {} , dp[10005] = {} , maxx[10005] = {} ;
bool flag = false ;
constexpr int val[22] = {0 , 100 , 100 , 97 , 96 , 95 , 90 , 85 , 80 , 82 , 78 , 75 , 70 , 65 , 55 , 50 , 15 , 12 , 10 , 8 , 5 , 2} ;
template<typename T>
inline T abs(T x)
{
	return x < 0 ? -x : x ;
...
int main()
{
	...
	for(int i = 1 ; i <= n ; ++i)
	{
		...
		for(int j = i - 1 ; j >= 1 ; --j)
		{
			if(abs(u[i] - u[j]) <= 2 * (t[i] - t[j] - 1) && t[i] > t[j] + 1)
				dp[i] = max(dp[i] , dp[j]) ;
			if(w[j] == 15)
				break ;
			if(t[i] - t[j] > 10)
		...
		dp[i] += val[w[i]] ;
		if(!flag)
			dp[i] = max(dp[i] , val[w[i]]) ;
		if(w[i] == 15)
			flag = true , maxx[i] = dp[i] ;
		else
