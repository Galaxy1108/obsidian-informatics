...
inline bool check()
{
	for(int i = 1 ; i <= n ; ++i) 
	{
		if(get[i] == i)
		{
			if(1.0 * c[i % n + 1] / (cnt[i % n + 1] + 1) > 1.0 * c[i] / cnt[i])
				return false ;
		}
		else
		{
			if(1.0 * c[i] / (cnt[i] + 1) > 1.0 * c[i % n + 1] / cnt[i % n + 1])
				return false ;
		}
	}
	return true ;
...
int main()
{
	...
	for(int i = 1 ; i <= n ; ++i)
	{
		if(1.0 * c[i] / (cnt[i] + 1) > 1.0 * c[i % n + 1] / (cnt[i % n + 1] + 1))
			get[i] = i , ++cnt[i] ;
		else
			get[i] = i % n + 1 , ++cnt[i % n + 1] ;
	}
	while(!check())
	{
		...
		for(int i = 1 ; i <= n ; ++i) 
		{
			...
			{
				if(1.0 * c[i % n + 1] / (cnt[i % n + 1] + 1) > 1.0 * c[i] / cnt[i])
					get[i] = i % n + 1 , --cnt[i] , ++cnt[i % n + 1] ;
			}
			else
			{
				if(1.0 * c[i] / (cnt[i] + 1) > 1.0 * c[i % n + 1] / cnt[i % n + 1])
					get[i] = i , --cnt[i % n + 1] , ++cnt[i] ;
			}
		}
	}
	for(int i = 1 ; i <= n ; ++i)
