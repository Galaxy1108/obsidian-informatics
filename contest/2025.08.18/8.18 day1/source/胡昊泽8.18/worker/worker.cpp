...
int dp[11][11][11][105] = {} ;
int a[105] = {} , n = 0 ;
int dfs(int A , int B , int C , int pos)
{
	if(pos > n)
		return (A ? 1 : 0) + (B ? 1 : 0) + (C ? 1 : 0) ;
	if(dp[A][B][C][pos] != inf)
		return dp[A][B][C][pos] ;
	int A_1 = A , B_1 = B , C_1 = C , i = 0 ;
	for(i = pos ; i < min(pos + (10 - A - B - C) , n + 1) ; ++i)
	{
		if(a[i] == 1)
			++A_1 ;
		else if(a[i] == 2)
	...
	if(A_1)
		dp[A][B][C][pos] = min(dp[A][B][C][pos] , dfs(0 , B_1 , C_1 , i) + 1) ;
	if(B_1)
		dp[A][B][C][pos] = min(dp[A][B][C][pos] , dfs(A_1 , 0 , C_1 , i) + 1) ;
	if(C_1)
		dp[A][B][C][pos] = min(dp[A][B][C][pos] , dfs(A_1 , B_1 , 0 , i) + 1) ;
	return dp[A][B][C][pos] ;
}
int main()
{
