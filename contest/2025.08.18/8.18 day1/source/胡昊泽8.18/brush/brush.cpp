...
int n = 0 , m = 0 ;
int val[10005] = {} ;
int dfs(int now , int pre1 , int pre2)
{
	if(dp[now][pre1][pre2] != 0x3f3f3f3f)
		return dp[now][pre1][pre2] ;
	if(now > m && pre2 <= 0)
		return 1e9 ;
	if(now > n)
		return 0 ;
	dp[now][pre1][pre2] = 1e9 ;
	dp[now][pre1][pre2] = min(dp[now][pre1][pre2] , dfs(now + 1 , pre1 - (pre1 > 0) , pre2 - (pre2 > 0))) ;
	dp[now][pre1][pre2] = min(dp[now][pre1][pre2] , dfs(now + 1 , m , pre1 - (pre1 > 0)) + val[now]) ;
	return dp[now][pre1][pre2] ;
}
int main()
{
